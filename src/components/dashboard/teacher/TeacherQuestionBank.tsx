...
const TeacherQuestionBank = ({ teacherId }: TeacherQuestionBankProps) => {
  ...
  const fetchQuestions = async () => {
    // Teachers see questions for their assigned subjects
          ...
          {subjects.length === 0 ? (
            ...
            <form onSubmit={handleAddQuestion} className="space-y-4">
              ...
              <div>
                ...
                <Textarea value={formData.question_text} onChange={(e) => setFormData({ ...formData, question_text: e.target.value })} maxLength={2000} required />
              </div>
                ...
                <RadioGroup value={formData.correct_answer} onValueChange={(v) => setFormData({ ...formData, correct_answer: v })} className="flex gap-4 mt-1">
                  {["A", "B", "C", "D"].map((opt) => (
                    <div key={opt} className="flex items-center gap-1">
                      <RadioGroupItem value={opt} id={`tbank-${opt}`} />
                      <Label htmlFor={`tbank-${opt}`}>{opt}</Label>
                    </div>
                  ))}
                </RadioGroup>
              </div>
